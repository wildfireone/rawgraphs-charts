...
export function render(
  ...
  link
    ...
    .attr("class", (d) => `link ${hashCode(d.source.id)} ${hashCode(d.target.id)}`)

  
  link
    .append('title')
    .text(d => `${d.source.id} → ${d.target.id}: ${d.value}`)
    //.attr("class", function(d) { console.table(d.source); });

    
    
    //link.append('title').text(d => `${d.source.name} → ${d.target.name}: ${d.value}`);
  
  function hashCode(stringvar) {
      var hash = 0,
        i, chr;
      if (stringvar.length === 0) return hash;
      for (i = 0; i < stringvar.length; i++) {
        chr = stringvar.charCodeAt(i);
